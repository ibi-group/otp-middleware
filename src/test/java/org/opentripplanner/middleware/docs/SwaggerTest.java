...
public class SwaggerTest extends OtpMiddlewareTest {
    ...
    private void rewritePathData(JsonNode pathsNode, JsonNode tagNode, String hostName) throws IOException {
        ...
        if (descriptionParts[0].equals("API_TEMPLATE")) {
            ...
            // Rewrite the /path, /path/{id}, and /path/fromtoken nodes.
            ObjectNode pathsObjectNode = (ObjectNode)pathsNode;
            pathsObjectNode.set(slashPath, getApiContent(apiGatewayPathTemplateFile, hostName, returnType, path));
            pathsObjectNode.set(slashPath + "/{id}", getApiContent(apiGatewayPathIdTemplateFile, hostName, returnType, path));
            pathsObjectNode.set(slashPath + "/fromtoken", getApiContent(apiGatewayPathTokenTemplateFile, hostName, returnType, path));

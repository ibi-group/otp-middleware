    ...
    public void bind(final SparkSwagger restApi) {
        ...
        apiEndpoint
            .get(path(ROOT_ROUTE)
                    ...
                    .withQueryParam().withName("userId").withRequired(true).withDescription("The OTP user for which to retrieve trip requests.").and()
                    .withQueryParam().withName(LIMIT_PARAM_NAME).withDescription(
                        String.format("If specified, the maximum number of trip requests to return, starting from the most recent. Defaults to %s.", DEFAULT_LIMIT)
                    ).and()
                    .withQueryParam().withName(FROM_DATE_PARAM_NAME).withDescription("If specified, the earliest date (format yyyy-MM-dd) for which trip requests are retrieved. Defaults to the current date.").and()
                    .withQueryParam().withName(TO_DATE_PARAM_NAME).withDescription("If specified, the latest date (format yyyy-MM-dd) for which usage logs are retrieved. Defaults to the current date.").and()
